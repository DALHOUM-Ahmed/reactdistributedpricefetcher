...
contract AXGT is ERC20, Ownable, ReentrancyGuard {
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private {
    uint256 tokens = (contractTokenBalance * liquidityFee) /
  ...
  function setSwapAtAmount(uint256 amount) external onlyOwner {
    require(
      amount <= (percentageOfMaximumTokensToAccumate * totalSupply()) / 100,
  ...
  function setAdminWallet(address _address) external onlyOwner {
    require(
      _address != address(0) && adminWallet != _address,
  ...
  function setFundWAllet(address _address) external onlyOwner {
    require(
      _address != address(0) && fundWallet != _address,
      "Invalid or same address provided"
    );
  ...
  function setLimit(uint256 _limit) external onlyOwner {
    require(
      _limit > 10 ** 6 * TOKEN_DECIMALS && balanceLimit != _limit,
  ...
  function setFee(
    ...
    require(
      0 < _newLPFee + _newadminFee + _newfundFee &&
        _newLPFee + _newadminFee + _newfundFee < 2500,
      "need to be strictly positive and smaller than 25%"
    );
